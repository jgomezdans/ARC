...
def arc_field(
    ...
    if siac_data_folder is not None:
        assert Path(siac_data_folder).exists(), (
            f"SIAC folder {siac_data_folder} does not exist."
        )
        s2_refs, s2_uncs, s2_angles, doys, mask, geotransform, crs = (
            get_s2_official_data(
                ...
                s2_end_date,
                geojson_path,
                siac_data_folder=siac_data_folder,
            )
        )
    else:
        s2_refs, s2_uncs, s2_angles, doys, mask, geotransform, crs = (
    ...
    if plot:
        random_inds = np.random.choice(s2_refs.shape[-1], 2)
        for i in random_inds:
            import matplotlib.pyplot as plt

            plt.figure(figsize=(24, 6))
            plt.plot(
                arc_refs[:, :, neighbours[i]].reshape(-1, 300),
                "-",
                color="b",
                lw=1,
                alpha=0.1,
    ...
    # Concatenate tensors
    scale_data = np.concatenate(
        [post_bio_scale_tensor, post_pheo_tensor, post_soil_tensor], axis=1
    )
